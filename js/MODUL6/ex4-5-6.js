//////////////////////////////////zad4/////////////////////////////////////
// filter и оператор !
// Получи массив только неактивных пользователей (отфильтруй 
// по значению свойства isActive)
...
//   // Write code under this line
const getInactiveUsers = array => array.filter(({isActive}) => {
  if (Object.isActive === false ? isActive : 'false');
    return !isActive;
});
...
//  Получи массив из объектов, которые состоят только из свойств name и 
//  email тех пользователей, которые попадают в возрастную категорию от 
...
//   const arr = Array(3)
//    .fill('')
//  .map((e,i) => ({['index']: i }));  // [ { index: 0 }, { index: 1 }, { index: 2 } ] */
 
   // Write code under this line
   const arr = [];
   const getUsersWithAge = (array, min, max) => array.filter(({age}) => {
    return (age > min && age < max)}).map(({name, email}) => 
   ({name, email}));  

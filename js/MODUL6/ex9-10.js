...
const copy = [...users]
const getNamesSortedByFriendsCount = (array) => copy.sort((a, b) => 
...
// не должно быть повторяющихся умений и они должны быть отсортированы в
// алфавитном порядке.
// Write code under this line
const getSortedUniqueSkills = (array) => array.reduce((skills, arr) => {
 return skills.concat(arr.skills)}, []).filter((item, i, skills) => { 
 return (skills.indexOf(item) === i)}).sort();

console.log(getSortedUniqueSkills(users)); //[ 'adipisicing', 'amet', 
// 'anim', 'commodo', 'culpa', 'elit', 'ex', 'ipsum', 'irure', 'laborum', 'lorem', 'mollit', 'non', 'nostrud',
